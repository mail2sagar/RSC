...
public class SearchFilter3 extends BrowserTypes {
	...
	By NominalVoltage = By.cssSelector("#filters > div > div > rs-facets > a.filter.filter-popover");
	By ThreeV  = By.cssSelector("#\\34 294845777 > span:nth-child(2)");
...
//Selecting the Batteries from the options	
	...
	//Selecting the camera batteries from the options	
		...
		public boolean NominalVoltage() {
			waitExplicitly(1);
			verbose("selecting Nominal Voltage from filter");
			getElement(NominalVoltage).click();
			return false;
		}
		//Selecting 3V from the list
		public boolean ThreeV() {
		//String actual_stock_no = null;
		waitExplicitly(1);
		verbose("selecting 3V from the filter");
		getElement(ThreeV).click();
		return false;
		}
